...
static void
fprop(const NeuralNetwork_Type nn,
...
static void
bprop(const NeuralNetwork_Type nn,
      const float *const in,
...
static void
wbrand(const NeuralNetwork_Type nn)
{
...
float *
NNpredict(const NeuralNetwork_Type nn,
          const float * in ){
    fprop(nn,in);
    return nn.o;
}
...
void
NNsave(const NeuralNetwork_Type nn,
       const char * path){
...
float
NNtrain(const NeuralNetwork_Type nn,
        const float * in,
        const float * tg,
        float rate){
...
float
NNProbabilityPrint(const float * arr,
                   const int size){
    double max =  0.0f;
    int idx;
...
int
NNPredictionPrint(const float * arr,
                  const int size){
    double max =  0.0f;
    int idx;
    for(int i=0;i <size;i++){
...
void
NNfree(const NeuralNetwork_Type nn){
    free(nn.w);
    free(nn.b);
    free(nn.h);
    free(nn.o);
